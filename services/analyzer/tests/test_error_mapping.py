...
def test_not_found_envelope(client, path, expected_status, expected_code):
    r = client.get(path)
...
def test_body_validation_failure_uses_the_same_envelope(client):
    r = client.post("/connections", json={"db_type": "sqlite"})
...
def test_unknown_db_type_is_a_validation_error(client):
    r = client.post(
...
def test_query_param_validation_failure(client):
    r = client.get("/queries/nope/logs", params={"limit": 0})
    assert r.status_code == 422
    assert r.json()["error_code"] == "REQUEST_VALIDATION_ERROR"
...
def test_validation_detail_is_json_serialisable(client):
    ...
    r = client.post("/connections", json={"name": "x", "db_type": "sqlite"})
    assert r.status_code == 422
    for entry in r.json()["detail"]["errors"]:
        assert set(entry) == {"loc", "msg", "type"}


def test_unhandled_exception_returns_500_without_leaking(client):
...
def test_unreachable_target_is_502(client, tmp_path):
    created = client.post(
        "/connections",
...
def test_no_schema_in_openapi_exposes_a_password_field(client):
